...
def _normalise_tag(tag: str) -> list[str]:
    ...
    tag = tag.lower().strip().replace(',', '').replace(';', '').replace('(', '').replace(')', '').replace('\u2026', '').replace('...', '').replace('"', '')
    tag = ' '.join(w.strip('-') for w in tag.split() if any(c.isalnum() for c in w) and not w.startswith("'"))
    parts = tag.split(" or ")
    if len(parts) > 1:
